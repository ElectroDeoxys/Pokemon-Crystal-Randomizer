...
class MoveSorter extends MoveAnalyser
{
    ...
    byte[] getTierMoveset(byte[][] lvlMoves, byte[] eggMoves, boolean[] moveComp, Type[] monTypes, int[] baseStats, byte lvl)
    {
        ...
        for (byte[] lvlMove : lvlMoves) // run through all the level moves
        {
            for (int i = 0; i < lvlMove.length; i++) // run through all the level moves
            {
                if (listMoves.contains(moves[byteToValue(lvlMove[i]) - 1]))
                {
                    continue;
                }
                listMoves.add(moves[byteToValue(lvlMove[i]) - 1]);
            }
        }
