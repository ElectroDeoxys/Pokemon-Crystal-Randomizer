...
class RomReader
{
    ...
    static byte[] readTextFromRom(FileChannel ch, int pos) throws IOException
    {
        ...
        ArrayList<Byte> textList = new ArrayList<>();
    ...
    PokemonGame[] readRomPokemon() throws IOException
    {
        ...
        ArrayList<Byte> futureEvos = new ArrayList<>(); // keep track of Pokemon after who have pre-evos
        ArrayList<Byte> futureEvosIndex = new ArrayList<>(); // store the Pokémon index that evolve into the evos
        ...
        for (int i = 0; i < N_POKEMON; i++)
        {
            ...
            {
                preEvoIndex[0] = futureEvosIndex.get(futureEvos.indexOf(valueToByte(i + 1)));
                hasPre = true;
            }
            ...
            {
                while (readByteFromRom(ch, pos) != (byte) 0x00) // read evolutions
                {
                    ...
                    if (byteToValue(evo[evo.length - 1]) < i + 1) // if evolution has already been processed
                    {
                        byte[] preEvoByte = new byte[1];
                        preEvoByte[0] = valueToByte(i + 1);
                        mons[byteToValue(evo[evo.length - 1]) - 1].setPreEvo(preEvoByte);
                    }
                    else // check afterwards to determine it's an evolution form and give it the index
                    {
                        futureEvos.add(evo[evo.length - 1]);
        ...
        for (int i = 0; i < N_POKEMON; i++) // loop again
        {
            ...
            if (mons[i].hasEvos()) // check the evolutions of this Pokemon to pass on the egg moves
            {
                ...
                for (byte[] e : evo) // cycle all evolutions
                {
                    PokemonGame evoMon = mons[byteToValue(e[e.length - 1]) - 1];
                    ...
                    if (evoMon.hasEvos()) // if that Pokemon has evos too, set their egg moves as well
                    {
                        ...
                        for (byte[] ee : evo2) // cycle all evolutions
                        {
                            PokemonGame evoMon2 = mons[byteToValue(ee[ee.length - 1]) - 1];
