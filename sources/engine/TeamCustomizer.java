...
class TeamCustomizer extends MoveAnalyser
{
	...
	{
		// assign roles
		ArrayList<ArrayList<Move>> movesets = new ArrayList<ArrayList<Move>>();
	...
	private ArrayList<Move> generateMoveset(PokemonGame mon, int lvl, Role role, ArrayList<Move> movepool, ArrayList<Move> lvlUpMoves)
	{
		...
		ArrayList<Move> movesMix = new ArrayList<Move>();
		...
		switch (role)
		{
			case PHYOFF 	  : moveset = basicOffenseMoveset(mon, maxTier, movesPhy, movesSta); break;
			case SPEOFF 	  : moveset = basicOffenseMoveset(mon, maxTier, movesSpe, movesSta); break;
			case MIXED  	  : moveset = basicOffenseMoveset(mon, maxTier, movesMix, movesSta); break;
			case PHYSWEEPER   : moveset = sweeperMoveset(mon, maxTier, movesPhy, movesSta); 	 break;
			case SPESWEEPER   : moveset = sweeperMoveset(mon, maxTier, movesSpe, movesSta); 	 break;
			case PHYTANK	  : moveset = tankMoveset(mon, maxTier, movesPhy, movesSta); 		 break;
			case SPETANK	  : moveset = tankMoveset(mon, maxTier, movesSpe, movesSta); 		 break;
			case SUPPORT	  : moveset = supportMoveset(mon, movesMix, movesSta); 	 			 break;
			case STALLER	  : moveset = stallerMoveset(mon, movesMix, movesSta); 	 			 break;
			case SLEEP_TALKER : moveset = sleepTalkerMoveset(mon, movesMix, movesSta);  		 break;
			case DREAM_EATER  : moveset = dreamEaterMoveset(mon, movesMix, movesSta);   		 break;
			case CURSER		  : moveset = curserMoveset(mon, maxTier, movesPhy, movesSta);   	 break;
			case FLAILER	  : moveset = flailerMoveset(mon, maxTier, movesPhy, movesSta);   	 break;
			default 		  : moveset = defaultMoveset(movepool);
		}
	...
	private void padMovepool(ArrayList<Move> moveset, ArrayList<Move> lvlUpMoves)
	{
		...
		for (Move m : lvlUpMoves)
		{
			...
			if (moveset.contains(m)) continue;
			if (getEffect(m) != MoveEffect.NO_EFFECT && hasMoveEffect(moveset, getEffect(m))) continue;
			moveset.add(m);
		}
	}
	...
	private void basicOffenseMoveset(ArrayList<Move> moveset, PokemonGame mon, int maxTier, ArrayList<Move> movesOff)
	{
		...
		{
			if (!hasNonSituational(curMovesSTAB)) break;
		...
		{
			if (!hasNonSituational(movesCvrg)) break;
		...
		{
			if (!hasNonSituational(movesNorm)) break;
	...
	private void basicOffenseMoveset(ArrayList<Move> moveset, PokemonGame mon, int maxTier, ArrayList<Move> movesOff, ArrayList<Move> movesSta)
	{
		...
		{
			if (!hasNonSituational(movesSta)) break;
	...
	private ArrayList<Move> sweeperMoveset(PokemonGame mon, int maxTier, ArrayList<Move> movesOff, ArrayList<Move> movesSta)
	{
		...
		if (moveset.size() >= 2)
		{
			...
			if (setupMove != null) movesToAdd.add(setupMove);

			// second, healing
			Move healMove = pickHealingMove(movesSta);
			if (healMove != null)  movesToAdd.add(healMove);

			// third, status inflicting
			Move statusMove = null;
			if (movesToAdd.size() == 0 || moveset.size() + movesToAdd.size() < 3)
				statusMove = pickStatusMove(movesSta);
			if (statusMove != null)  movesToAdd.add(statusMove);

			// fourth, +1 setup moves
			if (movesToAdd.size() == 0) // only add if none of the above were added
			{
	...
	private ArrayList<Move> tankMoveset(PokemonGame mon, int maxTier, ArrayList<Move> movesOff, ArrayList<Move> movesSta)
	{
		...
		Move statusMove = statusMove = pickStatusMove(movesSta);
		if (statusMove != null)  movesToAdd.add(statusMove);
	...
	private ArrayList<Move> supportMoveset(PokemonGame mon, ArrayList<Move> movesOff, ArrayList<Move> movesSta)
	{
		...
		if (moveset.size() < 4)
		{
			...
			if (moveOff != null) moveset.add(moveOff);
		}
	...
	private ArrayList<Move> stallerMoveset(PokemonGame mon, ArrayList<Move> movesOff, ArrayList<Move> movesSta)
	{
		...
		if (staMove != null && moveset.size() < 4)  movesToAdd.add(staMove);
	...
	private Move pickOffSetupMoveUp2(PokemonGame mon, ArrayList<Move> movesSta, boolean isSpecial)
	{
		...
		else if (setupMoveSpd != null && // only has speed setup
	...
	private Move pickHealingMove(ArrayList<Move> movesSta)
	{
		...
		if (healMove == null) healMove = pickMoveEffect(movesSta, MoveEffect.MORNING_SUN);
		if (healMove == null) healMove = pickMoveEffect(movesSta, MoveEffect.SYNTHESIS);
		if (healMove == null) healMove = pickMoveEffect(movesSta, MoveEffect.MOONLIGHT);
		return healMove;
	}
	...
	{	
		Move statusMove = null;
		if (statusMove == null) statusMove = pickMoveEffect(movesSta, MoveEffect.CAUSE_SLP);
		if (statusMove == null) statusMove = pickMoveEffect(movesSta, MoveEffect.CAUSE_PRZ);
		if (statusMove == null) statusMove = pickMoveEffect(movesSta, MoveEffect.CAUSE_CNF);
		return statusMove;
	}

	private ArrayList<ArrayList<Move>> getSTAB(PokemonGame mon, ArrayList<Move> movepool)
	{
		ArrayList<ArrayList<Move>> movesSTAB = new ArrayList<ArrayList<Move>>();
		Type[] monTypes = mon.getTypes();

		for (int i = 0; i < monTypes.length; i++)
		{
			if (i > 0 && monTypes[i] == monTypes[i-1]) break;
			ArrayList<Move> curMovesSTAB = new ArrayList<Move>();

			for (Move move : movepool)
			{
				if (move.getCalcPower() == 0) continue;
				if (!checkSTAB(monTypes[i], move)) continue;
	...
	{
		ArrayList<Move> movesCvrg = new ArrayList<Move>();
	...
	{
		ArrayList<Move> movesNorm = new ArrayList<Move>();
		ArrayList<Move> movesNormNoEffect = new ArrayList<Move>();
		...
		for (Move move : movepool)
		{
			...
			if (checkSTAB(mon, move)) break; // don't add for Normal-type
			movesNorm.add(move);
		}
	...
	private Move pickRandomMove(ArrayList<Move> movepool, boolean includeSituational)
	{
		...
		if (chosenMove != null) movepool.remove(movepool.indexOf(chosenMove));
	...
	private Move pickRandomMove(ArrayList<Move> movepool, int tier, boolean includeSituational)
	{
		...
		for (Move m : movepool)
		{
			if (m.getTier() < tier) break;
			if (!includeSituational && getEffect(m).situational()) continue;
	...
	private void fitMoveIn(Move move, ArrayList<Move> moveset)
	{
		...
		{
			ArrayList<Move> sameTierMoves = new ArrayList<Move>();
	...
	private void fitMovesIn(ArrayList<Move> moves, ArrayList<Move> moveset)
	{
		...
		for (Move mIn : moves)
		{
			if (moveset.contains(mIn)) continue;
			...
			else
			{
				...
				for (Move mS : moveset)
				{
					if (moves.contains(mS)) continue; // skip moves added by method
