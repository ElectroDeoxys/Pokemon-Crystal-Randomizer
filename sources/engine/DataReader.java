...
class DataReader
{
	...
	private PokemonData[] readPokemon(FileChannel chStats, FileChannel chEvoMoves, FileChannel chEggMoves, FileChannel chNames, FileChannel chMisc) throws IOException
	{
		...
		ArrayList<Integer> futureEvos = new ArrayList<Integer>(); // keep track of Pokemon after who have pre-evos
		ArrayList<Integer> futureEvosIndex = new ArrayList<Integer>(); // store the Pokémon index that evolve into the evos
		...
		for (int i = 0; i < N_POKEMON_DATA; i++)
		{
			...
			{
				preEvoIndex[0] = futureEvosIndex.get(futureEvos.indexOf(curIndex)).intValue();
				hasPre = true;
			}
	...
	Sprite[][] readDataSprites(FileChannel chF, FileChannel chB, FileChannel chPtr, 
							 ...
							 FileChannel chDim) throws IOException
	{
		...
		for (int i = 0; i < N_POKEMON_DATA; i++)
		{
			...
			if (i == INDEX_UNOWN - 1)
			{
				...
				int[] posNull = {0, 0}; // placeholder sprite offset
				...
				for (int j = 0; j < N_UNOWN; j++)
				{
					...
					int[] posNull1 = {0, 0}; // placeholder sprite offset
	...
	byte[][][] readDataPalettes(FileChannel ch) throws IOException
	{
