...
class PokemonEditor
{
	...
	void fitEggMoves(byte[] learnMoves)
	{
		...
		for (int i = 0; i < mons.length; i++)
		{
			...
			ArrayList<Byte> eggMovesList = new ArrayList<Byte>(eggMoves.length);
			
			for (byte eggMove : eggMoves)
				eggMovesList.add(eggMove);
			
			ArrayList<Byte> learnMovesList = new ArrayList<Byte>(learnMoves.length);
		...
		while ((size > SIZE_EGG_MOVES_MEM) && (count < mons.length))
		{
			...
			ArrayList<Byte> levelMovesList = new ArrayList<Byte>(levelMoves.length);
